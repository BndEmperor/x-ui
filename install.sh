...
if [[ $arch == "x86_64" || $arch == "x64" || $arch == "amd64" ]]; then
     ...
     arch="amd64"
     echo -e " ${red}Detect architecture failed, Use the default architecture: ${arch}${plain}"
fi
...
config_after_install() {
     echo -e "${yellow}For security reasons, the port and account password must be changed after the installation/update is complete ${plain}"
     ...
     if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
         ...
         echo -e "${red} has been cancelled, all settings are default settings, please modify in time ${plain}"
     fi
}
install_x-ui() {
     systemctl stop x-ui
     ...
     #echo -e ""
#https://raw.githubusercontent.com/BndEmperor/x-ui2/main/x-ui.sh
